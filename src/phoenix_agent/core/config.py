...
class AgentConfig:
    ...
    system_prompt: str = (
        ...
        "Available tools:\n"
        "  list_scheduled_tasks       — Show all configured scheduled tasks\n"
        "  add_scheduled_task         — Create a new scheduled task (requires restart to take effect)\n"
        "  remove_scheduled_task      — Remove a task by name (requires restart to take effect)\n\n"
        ...
        "3. Remind the user that server restart is needed for changes to take effect\n\n"
        "SKILLS:\n"
        "When a user's request matches a known skill (e.g. translation, code review, summarization),\n"
        "the skill is automatically activated and its prompt/tools are injected into your context.\n"
